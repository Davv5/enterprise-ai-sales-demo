...
const productNodes: StackNode[] = [
  {
    ...
    status: "configured",
    short: "Authenticated API + webhook façade",
    detail: "A Cloud Run service verifies inbound webhooks, maps provider objects into Harborline events, and exposes an authenticated command API. Secrets never reach the browser.",
  ...
  },
  {
    ...
    technology: "Pub/Sub + dead-letter topic",
    status: "configured",
    short: "Retryable async event handoff",
    detail: "Pub/Sub decouples ingestion, delivery, reply processing, and audit persistence. A dead-letter topic retains failed messages for controlled investigation.",
  ...
  },
  {
    ...
    technology: "Google Cloud Workflows",
    status: "configured",
    short: "Durable human-in-the-loop run",
    detail: "A durable workflow evaluates a versioned rule, pauses for a callback-backed approval, resumes only from a trusted approval event, and retries safe technical steps without re-sending communication.",
  ...
  },
  {
    ...
    status: "protected",
    short: "Schema-bound draft + classify only",
    detail: "Gemini can draft from an approved template or classify a reply into a typed exception under a restricted schema. It cannot set price, quantity, terms, eligibility exceptions, or delivery state.",
  ...
  },
  {
    id: "secrets",
    layer: "control",
    name: "Credentials & connector secrets",
    technology: "Secret Manager",
    status: "protected",
    short: "Versioned, server-only access",
    detail: "Secret Manager holds webhook keys, provider credentials, and connector configuration behind least-privilege service accounts. No secret value is bundled into the browser or committed to the repository.",
    reads: ["secret version reference", "service account identity"],
    writes: ["rotation and access audit"],
  },
  {
    id: "approval",
    layer: "control",
...
const outputNodes: StackNode[] = [
  ...
  },
  {
    ...
    detail: "Cloud SQL holds Harborline’s minimal operational records: external references, workflow runs, approvals, communication attempts, exceptions, and audit events. Source systems remain authoritative for their domain data.",
  ...
  },
  {
    ...
    technology: "Cloud SQL audit + Cloud Logging",
    status: "protected",
    short: "Correlated and redacted",
    detail: "Each transition shares a correlation ID, actor, rule/template version, and redaction posture. Cloud Logging supports diagnosis without exposing message content or credentials.",
