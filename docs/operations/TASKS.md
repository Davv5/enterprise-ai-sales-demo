...
- [x] Model a real drafted state for the Reorder workflow so "Record draft for Renee" records a draft event, not a simulated send, and the audit trail matches the clicks exactly. (Draft events now also persist in the trail after the send, in all workflows.)
- [x] Close the promo-gate URL loophole: no reachable state may show "Policy-gated distribution simulated" without a recorded gate event. (Gate state is derived for `demo=sent|exception`; `demo=drafted` on promo coerces to the review state; URLs self-normalize.)
- [x] Correct state-pill truthfulness: style the drafted pill, show "Needs assignment" until an exception is assigned, and use policy language for the promo post-gate state ("Policy gate recorded").
- [x] Fix the automation-mode card grid so mode descriptions no longer wrap one word per line in the 18px icon column.
- [x] Replace the stale browser-tab identity in `client/index.html`: Harborline title, honest description, working `favicon.svg` route-and-node mark; verified in dev and production build. (Also renamed the package to `harborline-communication-ops`.)
- [x] Load Manrope and IBM Plex Mono so the documented typography system actually renders. (Self-hosted via @fontsource so the demo works offline; verified with `document.fonts.check`.)
- [x] Restore navigation, Reset demo, and a persistent sandbox disclosure below 1024px; make Accounts reachable on mobile. (Compact command rail at top; verified at 375px and 768px.)
- [x] Reword the "LIVE DECISION RECORD" stamp to "ACTIVE DECISION RECORD".
- [x] Re-verify all runbook recovery URLs, typecheck, and production build; record a corrective decision-log entry for the mobile-verification claim.
- [ ] **Project-lead decision:** keep or remove the Manus runtime/analytics injection from client-facing pitch builds (privacy and payload trade-off documented in `FABLE_FIRST_REVIEW.md`).
- [ ] **Project-lead decision:** remove the non-functional search, notification, and status-filter chrome, or make it honest.
